/*
 *    Arduino_Exercises/Exercise16/Exercise16.ino
 *    
...
void ledDriver(char n, int on_ms, int off_ms)
{
  for (char i = 0; i < n; i++) {
...
void buzzerDriver(char n, int on_ms, int off_ms)
{
  for (char i = 0; i < n; i++) {
...
void setup()
{
  ...
  while (1) {
    if (digitalRead(SW2_PIN) == LOW) {
      buzzerDriver(1, 100, 100);
      while (1) {
        Serial.printf(
          "\n\r LL2=%d LL1=%d LR1=%d LR2=%d inside_offset=%d,outside_offset=%d",
          analogRead(LINE_L2_PIN), analogRead(LINE_L1_PIN), analogRead(LINE_R1_PIN),
          analogRead(LINE_R2_PIN), (analogRead(LINE_L1_PIN) - analogRead(LINE_R1_PIN)),
          (analogRead(LINE_L2_PIN) - analogRead(LINE_R2_PIN)));
        delay(100);
        if (digitalRead(SW2_PIN) == LOW) {
          delay(200);
          break;
