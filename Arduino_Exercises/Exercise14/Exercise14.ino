/*
 *    Arduino_Exercises/Exercise14/Exercise14.ino
 *    
...
void loop()
{
  ...
  if (pwm_l_value > 255) {
    pwm_l_value = 255;  //モーター制御値上限ガード処理
  }
  if (pwm_l_value <= 0) {
    pwm_l_value = 0;  //モーター制御値下限ガード処理
  }
  ...
  if (pwm_r_value > 255) {
    pwm_r_value = 255;  //モーター制御値上限ガード処理
  }
  if (pwm_r_value <= 0) {
    pwm_r_value = 0;  //モーター制御値下限ガード処理
  }
